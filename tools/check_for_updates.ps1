...
function Add-CurrentTime {
  Get-Date -Format d | Set-Content "$Env:OHMYPOSH_ROOT/.oh-my-posh-update"
}

if ($AutoUpdate -eq $true) {
  # Compare current date to last update date
  if (Test-Path "$Env:OHMYPOSH_ROOT/.oh-my-posh-update") {
    $oldDate = Get-Content "$Env:OHMYPOSH_ROOT/.oh-my-posh-update" | Get-Date -Format d
